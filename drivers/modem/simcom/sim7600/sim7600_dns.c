...
LOG_MODULE_REGISTER(modem_simcom_sim7600_dns, CONFIG_MODEM_LOG_LEVEL);
				   ...
				   const struct zsock_addrinfo *hints, struct zsock_addrinfo **res)
{
	...
	ret = snprintk(sendbuf, sizeof(sendbuf), "AT+CDNSGIP=\"%s\"", node);
	if (ret < 0) {
...
{
	if (recount > SIM7600_DNS_MAX_RECOUNT || timeout > SIM7600_DNS_MAX_TIMEOUT_MS) {
		return -EINVAL;
	}
