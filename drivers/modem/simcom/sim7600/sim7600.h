...
#ifndef SIMCOM_SIM7600_H
#define SIMCOM_SIM7600_H
...
#define MDM_APN CONFIG_MODEM_SIMCOM_SIM7600_APN
#define MDM_LTE_BANDS CONFIG_MODEM_SIMCOM_SIM7600_LTE_BANDS
...
enum sim7600_ftp_connection_state {
	/* Not connected yet. */
	SIM7600_FTP_CONNECTION_STATE_INITIAL = 0,
	/* Connected and still data available. */
	SIM7600_FTP_CONNECTION_STATE_CONNECTED,
	/* All data transferred. */
	SIM7600_FTP_CONNECTION_STATE_FINISHED,
	/* Something went wrong. */
	SIM7600_FTP_CONNECTION_STATE_ERROR,
};

enum sim7600_status_flags {
	SIM7600_STATUS_FLAG_POWER_ON = 0x01,
	SIM7600_STATUS_FLAG_CPIN_READY = 0x02,
	SIM7600_STATUS_FLAG_ATTACHED = 0x04,
	SIM7600_STATUS_FLAG_PDP_ACTIVE = 0x08,
};
...
struct sim7600_data {
	 ...
	 */
	enum sim7600_state state;
	enum sim7600_gnss_state gnss_state;
	/*
	 * RSSI work
	 */
	...
	struct {
		...
		enum sim7600_ftp_connection_state state;
	} ftp;
	/*
	 * Semaphore(s).
	 */
 ...
 * Driver internals
 */
extern struct sim7600_data mdata;
...
extern void sim7600_rssi_query_work(struct k_work *work);

enum sim7600_state sim7600_get_state(void);
enum sim7600_gnss_state sim7600_get_gnss_state(void);

void sim7600_change_state(enum sim7600_state state);
void sim7600_change_gnss_state(enum sim7600_gnss_state state);

int sim7600_pdp_activate(void);
int sim7600_pdp_deactivate(void);

int sim7600_offload_socket(int family, int type, int proto);

void sim7600_handle_sock_data_indication(int fd);

void sim7600_handle_sock_state(int fd, uint8_t state);

#endif /* SIMCOM_SIM7600_H */
