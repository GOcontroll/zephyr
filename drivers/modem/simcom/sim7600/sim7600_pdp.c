...
LOG_MODULE_REGISTER(modem_simcom_sim7600_pdp, CONFIG_MODEM_LOG_LEVEL);
...
MODEM_CMD_DEFINE(on_cmd_csq)
{
	...
	if (rssi == 0) {
		...
		mdata.mdm_rssi = -115;
	} else if (rssi > 101 && rssi < 191) {
		mdata.mdm_rssi = -114 + (rssi -102);
	} else if (rssi == 191) {
		mdata.mdm_rssi = -25;
	} else {
		mdata.mdm_rssi = -1000;
 ...
 */
void sim7600_rssi_query_work(struct k_work *work)
{
...
MODEM_CMD_DEFINE(on_cmd_cgatt)
{
	...
	if (cgatt) {
		mdata.status_flags |= SIM7600_STATUS_FLAG_ATTACHED;
	} else {
		mdata.status_flags &= ~SIM7600_STATUS_FLAG_ATTACHED;
	}
...
int sim7600_pdp_activate(void)
{
	int counter;
	int ret = 0;
#if defined(CONFIG_MODEM_SIMCOM_SIM7600_RAT_GSM)
...
#endif /* defined(CONFIG_MODEM_SIMCOM_SIM7600_RAT_GSM) */
	...
	while (counter++ < MDM_MAX_CGATT_WAITS &&
		(mdata.status_flags & SIM7600_STATUS_FLAG_ATTACHED) == 0) {
		ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, cgatt_cmd,
					 ARRAY_SIZE(cgatt_cmd), "AT+CGATT?", &mdata.sem_response,
					 MDM_CMD_TIMEOUT);
		if (ret < 0) {
	 ...
	 */
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0, "AT+NETOPEN",
				 &mdata.sem_response, MDM_CMD_TIMEOUT);
	if (ret < 0) {
	...
	if (ret < 0 || (mdata.status_flags & SIM7600_STATUS_FLAG_PDP_ACTIVE) == 0) {
	...
	sim7600_change_state(SIM7600_STATE_NETWORKING);
...
int sim7600_pdp_deactivate(void)
{
	...
	if (sim7600_get_state() != SIM7600_STATE_NETWORKING) {
		LOG_WRN("Cannot deactivate pdp context in state: %d", sim7600_get_state());
		goto out;
	}

	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0, "AT+NETCLOSE",
				 &mdata.sem_response, MDM_CMD_TIMEOUT);
	if (ret < 0) {
