...
LOG_MODULE_REGISTER(modem_simcom_sim7600_sock, CONFIG_MODEM_LOG_LEVEL);
...
static int offload_connect(void *obj, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	uint16_t dst_port = 0;
	char *protocol;
	struct modem_cmd cmd[] = { MODEM_CMD("+CIPOPEN: ", on_cmd_cipopen, 2U, ",") };
	char buf[sizeof("AT+CIPOPEN: #,#,#####,"
	...
	ret = snprintk(buf, sizeof(buf), "AT+CIPOPEN=%d,\"%s\",\"%s\",%d,-1", sock->id,
				  ...
				  const struct sockaddr *dest_addr, socklen_t addrlen)
{
	...
	if (len > 1500) {
		//error instead or multiple sends?
		len = 1500;
	}

	ret = snprintk(send_buf, sizeof(send_buf), "AT+CIPSEND=%d,%zu", sock->id, len);
	if (ret < 0) {
 ...
 *
 * The response has the form +CIPRXGET: <length>,data\r\nOK\r\n
 */
...
/*
 * Handler for ciprxget response.
 */
MODEM_CMD_DEFINE(on_cmd_ciprxget)
{
	return sockread_common(mdata.current_sock_fd, data, atoi(argv[2]), len);
}
...
{
	struct modem_socket *sock = (struct modem_socket *)obj;
	char sendbuf[sizeof("AT+CIPRXGET=#,##,####")];
	int ret, packet_size;
	struct socket_read_data sock_data;
	...
	snprintk(sendbuf, sizeof(sendbuf), "AT+CIPRXGET=2,%d,%zd", sock->id, max_len);
...
int mdm_sim7600_start_network(void)
{
	...
	if (sim7600_get_state() == SIM7600_STATE_NETWORKING) {
		...
		goto out;
	} else if (sim7600_get_state() != SIM7600_STATE_IDLE) {
