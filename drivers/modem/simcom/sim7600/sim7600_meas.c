...
LOG_MODULE_REGISTER(modem_sim7600_meas, CONFIG_MODEM_LOG_LEVEL);
...
static int cpsi_parse_gsm(struct sim7600_ue_sys_info *info, uint8_t **argv, uint16_t argc)
{
	int ret = -EINVAL;
...
static int cpsi_parse_lte(struct sim7600_ue_sys_info *info, uint8_t **argv, uint16_t argc)
{
	int ret = -EINVAL;
...
static struct sim7600_ue_sys_info *ue_sys_info;
...
MODEM_CMD_DEFINE(on_cmd_cpsi)
{
	...
	ue_sys_info->sys_mode = (enum sim7600_ue_sys_mode)ret;
	...
	ue_sys_info->op_mode = (enum sim7600_ue_op_mode)ret;

	if (ue_sys_info->sys_mode == SIM7600_UE_SYS_MODE_NO_SERVICE) {
		/* No further information available */
		ret = 0;
	} else if (ue_sys_info->sys_mode == SIM7600_UE_SYS_MODE_GSM) {
		ret = cpsi_parse_gsm(ue_sys_info, argv, argc);
	} else if (ue_sys_info->sys_mode == SIM7600_UE_SYS_MODE_LTE_CAT_M1 ||
			   ue_sys_info->sys_mode == SIM7600_UE_SYS_MODE_LTE_NB_IOT) {
...
int mdm_sim7600_get_ue_sys_info(struct sim7600_ue_sys_info *info)
{
	...
	if (sim7600_get_state() == SIM7600_STATE_OFF) {
		LOG_ERR("SIM7600 not powered on!");
		goto out;
	}
...
MODEM_CMD_DEFINE(on_cmd_cclk)
{
	...
	ret = sim7600_utils_parse_time(date, time_str, local_tm);
