...
LOG_MODULE_REGISTER(modem_sim7600_ftp, CONFIG_MODEM_LOG_LEVEL);
...
int mdm_sim7600_ftp_get_read(char *dst, size_t *size)
{
	...
	/* Some error occurred. */
	if (mdata.ftp.state == SIM7600_FTP_CONNECTION_STATE_ERROR ||
		mdata.ftp.state == SIM7600_FTP_CONNECTION_STATE_INITIAL) {
		return SIM7600_FTP_RC_ERROR;
	}
	...
	if (mdata.ftp.state == SIM7600_FTP_CONNECTION_STATE_FINISHED) {
		*size = 0;
		return SIM7600_FTP_RC_FINISHED;
	} else if (mdata.ftp.state == SIM7600_FTP_CONNECTION_STATE_ERROR) {
		*size = 0;
		return SIM7600_FTP_RC_ERROR;
	}
				  ...
				  const char *file, const char *path)
{
	...
	if (mdata.ftp.state != SIM7600_FTP_CONNECTION_STATE_CONNECTED) {
